...
// Un código está "en el catálogo" si coincide exactamente con una de las
// referencias de categoría. Cualquier otro valor (el código real de 33
// caracteres que asigna el SRI por marca/producto) se considera manual.
function esCodigoDeCatalogo(codigo: string) {
  return ICE_CATEGORIAS.some((c) => c.cod === codigo)
}
...
export default function ICEPage() {
  ...
  const [modoManual, setModoManual] = useState<Record<string, boolean>>({})
                      ...
                      return (
                        <tr key={p.id} className="border-t border-white/5 align-top">
                          ...
                          <td className="px-4 py-2.5">
                            {(modoManual[p.id] === true || (a.codigo_ice !== '' && !esCodigoDeCatalogo(a.codigo_ice))) ? (
                              <div className="flex flex-col gap-1 max-w-[260px]">
                                <input
                                  type="text"
                                  value={a.codigo_ice}
                                  onChange={(e) => setAsignaciones({ ...asignaciones, [p.id]: { ...a, codigo_ice: e.target.value } })}
                                  placeholder="Código ICE de 33 caracteres (SRI)"
                                  className="w-full rounded-md bg-white/5 border border-white/10 px-2 py-1 text-[11px] text-white font-mono outline-none focus:border-[var(--color-blue-5)]"
                                />
                                <button
                                  type="button"
                                  onClick={() => {
                                    setModoManual({ ...modoManual, [p.id]: false })
                                    setAsignaciones({ ...asignaciones, [p.id]: { ...a, codigo_ice: '', gramos_azucar: '0' } })
                                  }}
                                  className="text-left text-[10px] text-white/40 hover:text-white/70 hover:underline"
                                >
                                  ← Elegir de categorías
                                </button>
                              </div>
                            ) : (
                              <select
                                ...
                                onChange={(e) => {
                                  ...
                                  if (nuevoCodigo === '__manual__') {
                                    setModoManual({ ...modoManual, [p.id]: true })
                                    setAsignaciones({ ...asignaciones, [p.id]: { ...a, codigo_ice: '' } })
                                    return
                                  }
                              ...
                              >
                                ...
                                <option value="__manual__">✏ Otro — ingresar código real del SRI</option>
                              </select>
                            )}
                          </td>
                          <td className="px-4 py-2.5">
                          ...
                          <td className="px-4 py-2.5">
                            {(requiereGramosAzucar(a.codigo_ice) || modoManual[p.id] === true || (a.codigo_ice !== '' && !esCodigoDeCatalogo(a.codigo_ice))) ? (
                              <input
                                type="number"
                                step="0.01"
                                ...
                                className="w-24 rounded-md bg-white/5 border border-white/10 px-2 py-1 text-[11px] text-white outline-none focus:border-[var(--color-blue-5)]"
                              />
                            ) : (
